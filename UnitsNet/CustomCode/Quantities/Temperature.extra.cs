...
namespace UnitsNet
{
    public partial struct Temperature
    {
        ...
        public Temperature Multiply(decimal factor, TemperatureUnit unit)
        {
            decimal resultInUnit = As(unit) * factor;
        ...
        /// <param name="unit">Unit to perform multiplication in.</param>
        /// <returns>The resulting <see cref="Temperature" />.</returns>
        public Temperature Divide(decimal divisor, TemperatureUnit unit)
        {
            decimal resultInUnit = As(unit) / divisor;
            return From(resultInUnit, unit);
        }
    }
}
